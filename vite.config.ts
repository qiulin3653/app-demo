...
export default ({ command, mode }) => {
  ...
  return defineConfig({
    ...
    plugins: [
      UniPages({
        exclude: ['**/components/**/**.*'],
        routeBlockLang: 'json5', // 虽然设了默认值，但是vue文件还是要加上 lang="json5", 这样才能很好的格式化
      ...
      UNI_PLATFORM === 'h5' && {
        ...
        transformIndexHtml(html) {
          ...
          return html.replace('%BUILD_TIME%', dayjs().format('YYYY-MM-DD HH:mm:ss')).replace('%BUILD_BRANCH%', branch)
        },
      },
      // 打包分析插件，h5 + 生产环境才弹出
      UNI_PLATFORM === 'h5' &&
